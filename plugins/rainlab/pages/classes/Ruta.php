...
use RainLab\Pages\Classes\RutaList;
...
class Ruta extends ContentBase
{
    ...
    {
        $pages = Ruta::listInTheme($this->theme, true);
    ...
    {
        $rutaList = new RutaList($this->theme);
        $rutaList->appendPage($this);
    }
    ...
    {
        $eventList = new RutaList($this->theme);
        $eventList->removeSubtree($this);
    }
     ...
     * - chairs -> content/event-pages/chairs.htm
     *
    ...
    public function setDefaultLayout($parentPage)
    {
        ...
        if ($parentPage) {
            ...
            $component = $layout ? $layout->getComponent('staticRuta') : null;
        ...
        foreach (Layout::listInTheme($this->theme) as $layout) {
            $component = $layout->getComponent('staticRuta');
    ...
    public function getParent()
    {
        ...
        $parent = null;
        if ($fileName = $eventList->getPageParent($this)) {
    ...
    public function getChildren()
    {
        ...
        $subtree = $eventList->getPageSubTree($this);
    ...
    public function getLayoutOptions()
    {
        ...
        foreach ($layouts as $layout) {
            if (!$layout->hasComponent('staticRuta')) {
                continue;
            }
    ...
    {
        $key = crc32($theme->getPath()).'event-page-menu';
    ...
    public static function resolveMenuItem($item, $url, $theme)
    {
        ...
        if ($item->type == 'event-page' && !isset($tree[$item->reference])) {
            return;
        }

        $result = [];

        if ($item->type == 'event-page') {
        ...
        if ($item->nesting || $item->type == 'all-event-pages') {
            ...
            $result['items'] = $iterator($item->type == 'event-page' ? $pageInfo['items'] : $tree['--root-pages--']);
        }
    ...
    public static function buildMenuTree($theme)
    {
        ...
        $eventList = new RutaList($theme);
        $iterator($eventList->getPageTree(), null, 0);
    ...
    protected static function listStaticPageMenuOptions()
    {
        ...
        $eventList = new RutaList($theme);
        $pageTree = $eventList->getPageTree(true);
