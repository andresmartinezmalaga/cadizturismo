...
+function ($) { "use strict";
    ...
    var MultiLingual = function(element, options) {
        ...
        this.$dropdown.on('click', '[data-switch-locale]', function(event){
            ...
            // Andrés Martínez : Get multilingual wysiwyg elements
            var multilingualWysiwygElements = self.ams.multilingualGetWysiwygElements(event);
            var padreTemplate = multilingualWysiwygElements[0];
            var hijoActual = multilingualWysiwygElements[1];
            //----------------------------------------------------------------------------------
            ...
            // Andrés Martínez :  Insert template at markups multilingual when empty
            self.ams.multilingualInsertTemplate(padreTemplate,hijoActual);
            //----------------------------------------------------------------------------------------------
